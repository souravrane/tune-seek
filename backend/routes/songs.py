...
@router.post("/upload")
async def upload_song(file: UploadFile = File(...), title: str = Form("title"), artist: str = Form("artist"), url: str = Form("url"), image: str = Form("image")):
    ...
    # store in DB
    store_fingerprint(name = title, artist = artist, hashes = hashes, url = url, image = image)
...
async def match_audio(file: UploadFile = File(...)):
    ...
    # Extract fingerprints from query audio (only first 'duration' seconds)
